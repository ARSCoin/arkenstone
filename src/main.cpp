...
// Copyright (c) 2013-2014 Arkenstone developers
...
const string strMessageMagic = "Arkenstone Signed Message:\n";
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight <= 10) {nSubsidy = 400 * COIN;} // minimal rewards to prevent instamine
...
static const int64 nTargetSpacing = 15 * 60; // 15 Minutes
...
static const int64 nAveragingTargetTimespan = nAveragingInterval * nTargetSpacing; // 5 hours
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ArkenstoneMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ArkenstoneMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("ArkenstoneMiner started\n");
    ...
    try { loop {
        ...
        printf("Running ArkenstoneMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ArkenstoneMiner terminated\n");
        throw;
    }
}
